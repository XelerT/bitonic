...
try {
        ...
        config_t config {524'288, 32, 16, "../include/opencl/bitonic_sort.cl"};
        bitonic_sort_t<int> app(config);

        cl::vector<int> data(config.data_sz);
        cl::vector<int> sorted_data(data.size());

        rand_init(data.begin(), data.end(), 0, 0xFFF);


        t_start = std::chrono::high_resolution_clock::now();
        cl::Event evt = app.sort(data.data(), sorted_data.data(), data.size());
        ...
        GPU_t_start = evt.getProfilingInfo<CL_PROFILING_COMMAND_START>();
        GPU_t_fin   = evt.getProfilingInfo<CL_PROFILING_COMMAND_END>();
        long GPU_duration = (GPU_t_fin - GPU_t_start) / 1000000; // ns -> ms
        std::cout << "GPU pure time measured: " << GPU_duration << " ms" << std::endl;
